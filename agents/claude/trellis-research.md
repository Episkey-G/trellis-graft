...
2. **External Search** — load the `research` skill and follow it: investigate against **primary sources** (official docs, source code, specs, first-party APIs), never a secondary write-up, and follow every claim back to the source that owns it. Cite each claim.
...
For anything external, load the `research` skill first — it is the standard this project uses
for external investigation, and its primary-source discipline is what makes the output worth
persisting. Internal codebase search needs no skill; Glob/Grep/Read are enough.
